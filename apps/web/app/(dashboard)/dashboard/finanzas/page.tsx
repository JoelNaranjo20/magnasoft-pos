...
export default function CentralCashPage() {
    ...
    const nw = new Date();
    const monthStart = new Date(nw.getFullYear(), nw.getMonth(), 1).toISOString();
    const monthEnd = new Date(nw.getFullYear(), nw.getMonth() + 1, 1).toISOString();
    ...
    return (
        <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-[#0a0f14]">
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="w-full lg:w-[380px] flex-shrink-0 space-y-4">
                        <div className="bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-700 dark:to-slate-800 text-white rounded-2xl p-6 shadow-lg">
                            ...
                            {kpiLoading ? <div className="h-10 w-48 bg-white/10 rounded-lg animate-pulse mt-2" /> : <button onClick={() => setShowBalanceTotalModal(true)} className="text-left hover:opacity-80 transition-opacity cursor-pointer"><h2 className={`text-3xl font-black tabular-nums ${totalBalance >= 0 ? 'text-white' : 'text-rose-400'}`}>{totalBalance >= 0 ? '' : '−'}{formatCurrency(Math.abs(totalBalance))}</h2></button>}
                    ...
                    <div className="flex-1 space-y-4">
                                                                ...
                                                                <React.Fragment key={row.monthKey}>
                                                                    <tr className={`border-t border-slate-100 dark:border-slate-800 ${isMonthExpanded ? 'bg-blue-50/50 dark:bg-blue-900/10' : 'hover:bg-slate-50 dark:hover:bg-slate-800/30'} transition-colors`}>
                                                                        ...
                                                                        <td className="px-3 py-2 text-right tabular-nums"
                                                                            onClick={(e: any) => { if (e.ctrlKey || e.metaKey) { e.stopPropagation(); startEdit(row.monthKey, 'ingresos', row.ingresos); } }}>
                                                                            {editingCell?.monthKey === row.monthKey && editingCell?.field === 'ingresos' ? (
                                                                                <input type="number" value={editValue} data-edit={`${row.monthKey}-ingresos`}
                                                                                    ...
                                                                                    onKeyDown={(e: any) => { if (e.key === 'Enter') saveEdit(); if (e.key === 'Escape') setEditingCell(null); }}
                                                                                    onBlur={saveEdit}
                                                                        ...
                                                                        {/* Egresos */}
                                                                        <td className="px-3 py-2 text-right tabular-nums"
                                                                            onClick={(e: any) => { if (e.ctrlKey || e.metaKey) { e.stopPropagation(); startEdit(row.monthKey, 'egresos', row.egresos); } }}>
                                                                        ...
                                                                        {/* Bonos */}
                                                                        <td className="px-3 py-2 text-right tabular-nums"
                                                                            onClick={(e: any) => { if (e.ctrlKey || e.metaKey) { e.stopPropagation(); startEdit(row.monthKey, 'bonos', row.bonos); } }}>
                                                                        ...
                                                                        {/* Servicios */}
                                                                        <td className="px-3 py-2 text-right tabular-nums"
                                                                            onClick={(e: any) => { if (e.ctrlKey || e.metaKey) { e.stopPropagation(); startEdit(row.monthKey, 'servicios', row.servicios); } }}>
                                                                        ...
                                                                        <td className={`px-3 py-2 text-right font-black tabular-nums ${row.neto >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                                                                            onClick={(e: any) => { if (e.ctrlKey || e.metaKey) { e.stopPropagation(); startEdit(row.monthKey, 'neto', row.neto); } else if (hasDetail) { toggleMonthDetail(row.monthKey); } }}>
                        ...
                        <div className="bg-white dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-5">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button onClick={() => setShowAcreedoresTotalModal(true)} className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/30 rounded-xl p-3 text-left hover:bg-amber-100 dark:hover:bg-amber-900/20 transition-colors group"><p className="text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase mb-1">Deuda Total</p>{acreedoresLoading ? <div className="h-6 w-20 bg-amber-100 dark:bg-amber-900/20 rounded animate-pulse" /> : <p className="text-lg font-black text-amber-700 dark:text-amber-300 tabular-nums">{formatCurrency(acreedoresTotal)}</p>}<p className="text-[8px] text-amber-500/60 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">Ver detalle →</p></button>
                                <button onClick={() => setShowAcreedoresPagosModal(true)} className="bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-800/30 rounded-xl p-3 text-left hover:bg-rose-100 dark:hover:bg-rose-900/20 transition-colors group"><p className="text-[10px] font-bold text-rose-600 dark:text-rose-400 uppercase mb-1">Pagado del Mes</p>{acreedoresLoading ? <div className="h-6 w-20 bg-rose-100 dark:bg-rose-900/20 rounded animate-pulse" /> : <p className="text-lg font-black text-rose-700 dark:text-rose-300 tabular-nums">{formatCurrency(acreedoresPagadoMes)}</p>}<p className="text-[8px] text-rose-500/60 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">Ver detalle →</p></button>
                            </div>
                        </div>
            ...
            {/* Balance Total — Movimientos del mes */}
                ...
                <div className="fixed inset-0 z-[310] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="relative w-full max-w-2xl max-h-[85vh] bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden flex flex-col">
                        <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
                            <div><h2 className="text-lg font-bold text-slate-900 dark:text-white">💰 Balance Total — Movimientos del Mes</h2><p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{movimientosDelMes.length} movimientos · Balance: <span className={`font-bold ${totalBalance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{totalBalance >= 0 ? '' : '−'}{formatCurrency(Math.abs(totalBalance))}</span></p></div>
                            <button onClick={() => { setShowBalanceTotalModal(false); setEditingMov(null); }} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"><span className="material-symbols-outlined !text-[20px]">close</span></button>
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar px-6 py-4 space-y-1">
                            {loading ? (
                                ...
                                <div className="flex flex-col items-center justify-center py-12 text-slate-400"><span className="material-symbols-outlined !text-[48px] opacity-30">receipt_long</span><p className="text-sm font-semibold mt-2">Sin movimientos este mes</p></div>
                            ) : (
                                movimientosDelMes.map((m: any) => {
                                    const isIncome2 = m.type === 'income';
                                    const isEditing2 = editingMov === m.id;
                                    const methodLabel2 = m.payment_method === 'transfer' || m.payment_method === 'card' ? '🏦 Transf.' : m.payment_method === 'mixed' ? '🔀 Mixto' : '💵 Efectivo';
                                    const dateStr2 = new Date(m.created_at).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' });
                                    return (
                                        <div key={m.id} className={`flex items-center gap-3 py-2.5 px-3 rounded-lg transition-colors ${isEditing2 ? 'bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800' : 'hover:bg-slate-50 dark:hover:bg-slate-700/30'}`}>
                                            {isEditing2 ? (
                                                <div className="flex-1 flex flex-wrap items-center gap-2">
                                                    <select value={editMovType} onChange={(e: any) => setEditMovType(e.target.value)} className="text-xs font-bold px-2 py-1 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white"><option value="income">Ingreso</option><option value="expense">Egreso</option></select>
                                                    <input type="number" value={editMovAmount} onChange={(e: any) => setEditMovAmount(e.target.value)} className="w-28 text-xs font-bold px-2 py-1 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white" onKeyDown={(e: any) => { if (e.key === 'Enter') handleSaveEditMov(); if (e.key === 'Escape') setEditingMov(null); }} />
                                                    <input type="text" value={editMovDesc} onChange={(e: any) => setEditMovDesc(e.target.value)} className="flex-1 min-w-[150px] text-xs font-medium px-2 py-1 rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white" onKeyDown={(e: any) => { if (e.key === 'Enter') handleSaveEditMov(); if (e.key === 'Escape') setEditingMov(null); }} />
                                                ...
                                                <>
                                                    <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${isIncome2 ? 'bg-emerald-500' : 'bg-rose-500'}`} />
                                                    <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate">{m.description || 'Movimiento'}</p><p className="text-[10px] text-slate-400">{dateStr2} · {methodLabel2}</p></div>
                                                    <span className={`text-sm font-black tabular-nums flex-shrink-0 ${isIncome2 ? 'text-emerald-600' : 'text-rose-600'}`}>{isIncome2 ? '+' : '−'}{formatCurrency(m.amount)}</span>
                                                    <button onClick={(e: any) => { e.stopPropagation(); startEditMov(m); }} className="p-1 rounded hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"><span className="material-symbols-outlined !text-[16px]">edit</span></button>
                                                    <button onClick={(e: any) => { e.stopPropagation(); handleDeleteMov(m.id); }} className="p-1 rounded hover:bg-red-100 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-600 transition-colors"><span className="material-symbols-outlined !text-[16px]">delete</span></button>
                                                </>
                                            )}
                                        </div>
                                    );
                        ...
                        <div className="flex justify-end px-6 pb-5 pt-2 border-t border-slate-200 dark:border-slate-700 flex-shrink-0"><button onClick={() => { setShowBalanceTotalModal(false); setEditingMov(null); }} className="px-6 py-2.5 text-sm font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">Cerrar</button></div>
                    </div>
                </div>
            )}
