...
export const CentralCash = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        console.log('🔵 Form submitted', { amount, description, type });

        const val = parseFloat(amount);
        console.log('🔵 Parsed amount:', val);

        if (!val || val <= 0 || !description) {
            console.warn('⚠️ Validation failed:', { val, description });
            alert('Por favor completa todos los campos correctamente');
            return;
        }

        console.log('🔵 Calling addMovement...');
        setProcessing(true);
        const res = await addMovement(type, val, description);
        setProcessing(false);
        console.log('🔵 addMovement result:', res);
    ...
    return (
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col md:flex-row min-h-[600px]">
            ...
            <div className="w-full md:w-1/3 p-8 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col">
                ...
                <div className="bg-slate-900 text-white rounded-[2rem] p-6 shadow-xl mb-8 relative overflow-hidden group flex-shrink-0">
                    ...
                    <h2 className="text-4xl font-black tabular-nums relative z-10 break-all">${balance.toLocaleString()}</h2>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 flex-1 flex flex-col">
                    ...
                    <div className="flex-grow flex flex-col min-h-0">
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:border-primary font-bold text-slate-900 dark:text-white resize-none flex-grow min-h-[100px]"
                            required
                        />
                    </div>
                    ...
                    <button
                        ...
                        className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-2 flex-shrink-0 ${type === 'income' ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-rose-500 hover:bg-rose-600 text-white shadow-lg shadow-rose-500/20'}`}
                    >
                        {processing ? (
            ...
            <div className="w-full md:w-2/3 p-8 flex flex-col h-full bg-slate-50 dark:bg-slate-900/50">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 max-h-[600px]">
                        ...
                        movements.map((mov) => (
                            <div key={mov.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between group hover:border-primary/20 transition-all">
                                <div className="flex items-center gap-4 min-w-0 flex-1">
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${mov.type === 'income' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-500' : 'bg-rose-50 dark:bg-rose-900/20 text-rose-500'}`}>
                                        <span className="material-symbols-outlined !text-2xl">
                                            {mov.type === 'income' ? 'trending_up' : 'trending_down'}
                                        </span>
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <p className="font-black text-slate-800 dark:text-slate-200 leading-none mb-1 truncate" title={mov.description}>{mov.description}</p>
