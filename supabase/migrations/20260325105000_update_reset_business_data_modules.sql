-- Create or Replace the RPC function to reset specific business data modules securely
...
BEGIN
    ...
    -- 4. Clear Customer Data (Clientes y vehículos)
    ...
    -- 5. Clear Workers Data (Trabajadores y comisiones)
    ...
    -- 6. Clear Products and Services
    IF p_delete_products THEN
        -- Products have categories, delete categories? Maybe keep categories? 
        -- Safer to delete products and services, but maybe not categories. Given the broad request, we delete products and services.
        DELETE FROM public.products WHERE business_id = p_business_id;
        DELETE FROM public.services WHERE business_id = p_business_id;
    END IF;

    -- 7. Clear Service Queue
